...
2. 与输入数据对应的文本，例如上图对应的文本是：`(0+0)+9=9`这一长度为10的字符序列。
...
另外，考虑到数据集中图像的颜色差异很大，并且彩色通道对特征提取没有帮助（因为颜色信息本身与标签没有实质关系），因此将考虑将数据灰度化后再输入到模型。
